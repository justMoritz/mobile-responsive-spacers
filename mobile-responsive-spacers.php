...
class Maz_Mrs_Fields_Plugin {
  ...
  public function maz_mrs_field_callback_breakpoint( $arguments ) {
    ...
    echo '<input name="maz_mrs_breakpoint" id="maz_mrs_breakpoint" step="1" type="number" value="' . get_option( 'maz_mrs_breakpoint' ) . '" />';
  }
  ...
  public function maz_mrs_field_callback_ratio( $arguments ) {
    ...
    echo '<input name="maz_mrs_ratio" id="maz_mrs_ratio" step="0.01" type="number" value="' . get_option( 'maz_mrs_ratio' ) . '" />';
  }
  ...
  public function maz_mrs_settings_content() {
    ?>
      <div class="wrap">
        ...
        <div class="">
          A Plugin by <a href="https://moritzzimmer.com">Moritz Zimmer</a>, <?=Date('Y')?>
        </div>
      </div>
    <?php
    }
...
function maz_halve_spacers_on_mobile_init() {
  ...
  $phpInfo = array(
    'breakpoint' => get_option( 'maz_mrs_breakpoint' ),
    'ratio' => get_option( 'maz_mrs_ratio' )
  );

  wp_enqueue_script('maz_mrs_scripts', plugin_dir_url( __FILE__ ) . 'assets/mobile-responsive-spacers.js', ['jquery'], $gloablVersion, true);
  wp_localize_script( 'maz_mrs_scripts', 'mazMrsVars', $phpInfo );
}
...
function add_maz_mrs_action_links ( $links ) {
    $settingslinks = array(
      '<a href="' . admin_url( 'options-general.php?page=maz_mrs_fields' ) . '">Settings</a>',
    );
